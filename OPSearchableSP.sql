...
CREATE Procedure OutP3MCoders @startDate varchar(10)= NULL, @endDate varchar(10)=NUll
AS
...
select distinct [dbo].[OutPatient_Encoder_ID].Encoder_ID as [Encoded_By]
	...
	,[Hourly Productivity] = CAST((ISNULL(A.BillCount,0)/((5.9 * (Z.DW)) + isnull((Z.[OT_Hours]+ Z.PtD),0)))AS decimal(18,2))
	, Z.[Reg-1]
	, Z.[Reg-2]
	, Z.[OT_Hours]
	, Z.TotalHrs
