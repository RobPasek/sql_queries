...
FROM   MSP_EpmProject_UserView AS P 
			INNER JOIN (SELECT MSP_EpmProject_UserView.ProjectUID, ISNULL(STUFF
						((SELECT ', ' + MemberValue
						...
						ON [MSPLT_BusinessOwnerLT_UserView].LookupMemberUID = [MSPCFPRJ_Business Owner_AssociationView].LookupMemberUID
						 ...
						 LEFT JOIN (SELECT I.ProjectUID, SUM(IIF(I.Priority = '01 Critical', 1000, IIF(I.Priority = '02 High', 1, 0))) AS [PrioritySum]
									FROM MSP_WssIssue I
									GROUP BY i.ProjectUID) AS IP ON IP.ProjectUID = P.ProjectUID 
									LEFT JOIN (SELECT R.ProjectUID
												, SUM(IIF(R.Exposure > 7.5, 1000
												, IIF(R.Exposure > 5 AND R.Exposure < 7.5, 1, 0))) AS [RiskSum]
									FROM MSP_WssRisk R GROUP BY R.ProjectUID) AS RP 
									ON RP.ProjectUID = P.ProjectUID