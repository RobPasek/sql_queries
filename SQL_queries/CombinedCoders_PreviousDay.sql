...
SELECT dbo.myUnion_Encoder_ID.Encoder_ID
	  ...
	  , [IpProductivity] = CASE 
			WHEN ['Position_Staff - Coder$'].EE# = '5121' 
				then 
					cast(round(dbo.SoftMed_InPatient_Only_Sums_Tbl_PreviousDay.[InPatient Productivity]/(ISNULL[dbo].[PayCodeSum_Daily].[Reg-2] ,0) AS [Total REG-2]+ISNULL([dbo].[PayCodeSum_Daily].[Total OT],0)),2,0) as decimal(18,2))
			CASE WHEN ['Position_Staff - Coder$'].EE# = '5121' 
				then 
					cast(round(dbo.SoftMed_InPatient_Only_Sums_Tbl_PreviousDay.[InPatient Productivity]/(ISNULL[dbo].[PayCodeSum_Daily].[Reg-2] ,0) AS [Total REG-2]+ISNULL([dbo].[PayCodeSum_Daily].[Total OT],0)),2,0) as decimal(18,2))
