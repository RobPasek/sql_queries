SELECT        B.ProjectUID
			, B.Title, B.AssignedToResource
			,CASE 
				WHEN B.Status = '(1) Active' THEN 'Active' 
				WHEN B.Status = '(2) Postponed' THEN 'Postponed' 
				WHEN B.Status = '(3) Closed' THEN 'Closed' 
				ELSE B.Status
            END AS Status
			,CASE 
				WHEN B.Category = '01 Resources' THEN 'Resources' 
...
FROM            WSS_Content_CHOC.dbo.AllUserData AS A INNER JOIN
                         MSP_WssRisk AS B ON A.tp_DocId = B.RiskUniqueID
