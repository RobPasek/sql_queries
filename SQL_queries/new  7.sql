...
SELECT [dbo].[myUnion_Encoder_ID].Encoder_ID as [Encoded_By]
	  ...
	  , [IpProductivity] = ISNULL([dbo].[TestTable].[InPatient_Productivity]/((5.9 * (A.DW)) + isnull((A.[OT_Hours]),0)),0)
	  , [OpProductivity] = ISNULL([dbo].[TestTable].[OutPatient_Productivity]/((5.9 * (A.DW)) + isnull((A.[OT_Hours]),0)),0)
