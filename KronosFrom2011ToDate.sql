CREATE PROCEDURE ThreeM_Data_Tables
AS 


SELECT        visit_view.discharged, ctc_visit.visit_detail_pt_type, ctc_encoderresults_view.enc_code_dt_tm, ctc_encoderresults_init_view.enc_code_dt_tm AS Encode_Date, 
                         ctc_record_hstry_view_latest_FNL.rec_hstry_trans_date_time, ctc_visit.is_finalized, LOWER(ctc_encoderresults_init_view.enc_code_id) AS Encoder_ID, 
                         LOWER(ctc_encoderresults_view.enc_code_id) AS Encoded_By, visit_view.bill_no, visit_view.mrn, visit_view.last_name, visit_view.first_name, 
                         visit_view.length_of_stay, ctc_encoderresults_view.enc_drg_weight
into dbo.[3M_2011_to_Present]
...
WHERE        (cast(ctc_encoderresults_view.enc_code_dt_tm as date) >= '2011') AND (ctc_encoderresults_view.enc_code_dt_tm < CONVERT(date, 
                         GETDATE()))
GO