...
set @fiscalyear = '2015-06-30'--cast(getdate() as date)
...
FROM            MSP_EpmProject_UserView AS P 
				INNER JOIN (SELECT ProjectUID, taskname,TaskIsMilestone, iif(TaskFinishDate <= TaskBaseline0FinishDate,1,0) AS test 
				FROM MSP_EpmTask_UserView WHERE TaskFinishDate <= TaskBaseline0FinishDate AND TaskIsMilestone = 1) AS T 
				ON P.ProjectUID = T.ProjectUID 
				/*INNER JOIN MSP_WssRisk AS R 
				ON P.ProjectUID = R.ProjectUID*/
WHERE        /*(T.TaskIsMilestone = 1) 
			--AND (P.ProjectPercentCompleted = 100) 
			and*/ (P.ProjectFinishDate >= '07/01/' + CONVERT(nchar(4), dbo.fnc_FiscalYear(@fiscalyear))) 
